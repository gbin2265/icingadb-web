...
/** @implements ItemTableRenderer<HostservicesSummary> */
class HostservicesRenderer implements ItemTableRenderer
{
    use Translation;
    use BaseFilter;
    ...
    {
        $attributes->get('class')->addValue('host-services');
    }
        ...
        if ($layout === 'header') {
            ...
            $link = new Link(
                ...
                [
                    'class' => 'subject',
                    'title' => sprintf(
                        $this->translate('List all Host Services "%s"'),
                        $item->display_name
                    )
                ]
            );
     ...
     *
     * @param HostservicesSummary $item
     *
     * @return ServiceStatistics
     */
    protected function createStatistics(HostservicesSummary $item): ServiceStatistics
    {
        return (new ServiceStatistics($item))
            ->setBaseFilter(Filter::equal('host.name', $item->name));
    }
}
